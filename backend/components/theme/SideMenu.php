...
class SideMenu extends \yii\widgets\Menu
{
    ...
    public function init()
    {
        ...
        $this->labelTemplate = '{icon}<span>{label}</span>';
        $this->markTopItems();
    ...
    protected function renderItem($item)
    {
        ...
        if (empty($item['top'])) {
            if (!empty($item['items'])) {
                $template = isset($item['template']) ? $item['template'] :"<a href='{url}' class='auto'>{icon}{label}</a>";
                ...
                $indicator = Html::tag('span',"<i class='fa fa-fw fa-angle-right text'></i><i class='fa fa-fw fa-angle-down text-active'></i>",array("class"=>'pull-right text-muted'));
